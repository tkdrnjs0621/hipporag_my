...
def named_entity_recognition(passage: str):
    ...
    while not_done:
        try:
            ...
            elif isinstance(client, ChatOllama) or isinstance(client, ChatLlamaCpp)or isinstance(client, ChatHuggingFace):
                response_content_ = client.invoke(ner_messages.to_messages())
                # print(response_content.content)
                response_content = extract_json_dict(response_content_.content)
                total_tokens += len(response_content_.content.split())
            else:  # no JSON mode
...
def openie_post_ner_extract(passage: str, entities: list, model: str):
    ...
    try:
        if isinstance(client, ChatOpenAI):  # JSON mode
            ...
            response_content_ = client.invoke(openie_messages.to_messages())
            response_content = extract_json_dict(response_content_.content)
            response_content_ = str(response_content_.content)
            total_tokens = len(response_content_.split())
        else:  # no JSON mode
...
if __name__ == '__main__':
    ...
    if(dataset=="hotpotqa"):
        tx='output/openie_hotpotqa_results_ner_meta-llama_Llama-3-8b-chat-hf_9221.json'
    elif(dataset=="musique"):
        tx='output/openie_musique_results_ner_meta-llama_Llama-3-8b-chat-hf_11656.json'
    else:
        tx='output/openie_2wikimultihopqa_results_ner_meta-llama_Llama-3-8b-chat-hf_6119.json'
    print("TX :",tx)
    auxiliary_files = glob(tx)
